...
export const init = (root) => {
    ...
    // Event handler to support pressing enter on groups filter popover actions.
    $('#filter-groups-popover').on("keydown", ".filter-body", function(event) {
        if ((event.charCode === 13 || event.keyCode === 13) && event.target.firstChild.classList.length > 0) {
            event.preventDefault();

            switch(event.target.firstChild.classList[0]) {
                case 'select-all':
                    selectAll('filter-groups-popover');
                    break;
                case 'filter-clear':
